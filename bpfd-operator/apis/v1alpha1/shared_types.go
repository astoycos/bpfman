...
func IsBpfProgramConditionFailure(conditionType string) bool {
	if conditionType == string(BpfProgCondNotLoaded) ||
		conditionType == string(BpfProgCondNotUnloaded) ||
		conditionType == string(BpfProgCondMapOwnerNotFound) ||
		conditionType == string(BpfProgCondMapOwnerNotLoaded) {
		return true
	}
