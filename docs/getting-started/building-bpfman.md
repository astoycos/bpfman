...
* **Relaxed CAP_BPF Requirement:** Prior to Kernel 5.19, all eBPF sys calls required CAP_BPF.
  This required userspace programs that wanted to access maps shared between the BFP program and the
  userspace program to have the CAP_BPF Linux capability.
