...
///
/// If a namespace_pid is provided, attach it to malloc() in that namespace.
...
impl DockerContainer {
    /// Return the namespace PID of the container
...
pub fn start_container() -> Result<DockerContainer> {
    ...
    let mut namespace_pid: i32 = 0;
    for line in output.lines() {
        if line.contains("nginx") {
            let pid_str: Vec<&str> = line.split_whitespace().collect();
            namespace_pid = FromStr::from_str(pid_str[3]).unwrap();
            break;
        }
    }
    assert!(namespace_pid != 0);

    debug!(
        "Docker container {} with namespace PID {} created",
        container_id, namespace_pid
    );
